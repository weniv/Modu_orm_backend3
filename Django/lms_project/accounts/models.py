...
class User(AbstractUser):
    ...
    role = models.CharField(
        max_length=20, choices=ROLE_CHOICES, default="USER", verbose_name="역할"
    )
    ...
    def can_borrow(self):
        ...
        active_loan = Loan.objects.filter(user=self, status="ACTIVE").count()
    ...
    def has_overdue_books(self):
        ...
        return Loan.objects.filter(
            user=self, status="ACTIVE", due_date__lt=timezone.now()
        ).exists()
