...
def loan_create(request, book_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(request, f'{book.title} 도서가 대출되었습니다. 반납일은 {
                             loan.due_date.date()}입니다.')
    ...
    if request.method == 'POST':
        if loan.status == 'ACTIVE':
            ...
            waiting_reservation = Reservation.objects.filter(
                book=loan.book, status='WAITING').order_by('reserved_date').first()
...
def overdue_list(request):
    loans = Loan.objects.filter(
        status='ACTIVE', due_date__lt=timezone.now()).order_by('due_date')
    return render(request, 'books/loan_list.html', {'loans': loans, 'show_overdue': True})
...
def reservation_list(request):
    ...
    return render(request, 'books/reservation_list.html', {'reservations': reservations})
